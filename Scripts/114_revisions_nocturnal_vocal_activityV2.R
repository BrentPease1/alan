  ...
  for (m in 1:nrow(months)) {
    ...
    for (dets in detection_filter) {
      ...
      if (overwrite == TRUE) {
        ...
        if (file.exists(
          paste0(
            here('Results/Birdweather/nocturnalV2'),
            "/",
            c,
            "/nocturnal_measures_",
            this_month,
      ...
      for (f in focal_spp) {
        ...
        single_spp_dets[, min_time_nadir := apply(.SD, 1, function(x) x[which.min(abs(x))]), 
                        .SDcols = c("time_since_nadir", "time_since_pre_nadir", "time_since_post_nadir"), station_date]
        
        out <- single_spp_dets[, .(sci_name, com_name, min_time_nadir = min(min_time_nadir), date, week, lat, lon), station_date]
        out <- out[!duplicated(station_date)]
        #out <- single_spp_dets[!duplicated(station_date), .(com_name,sci_name, date_time, date, week, lat, lon, station_date, min_time_nadir)]
      ...
      ifelse(!dir.exists(file.path(paste0(
        here('Results/Birdweather/nocturnalV2'), "/", c
