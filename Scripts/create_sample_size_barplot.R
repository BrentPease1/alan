...
final <- holder |> 
  ...
  # dplyr::mutate(across(unfilt:response, function(x) log(x))) |>
  ...
  ggplot(final, aes(x = month_year)) +
    ...
    # geom_bar(aes(y = unfilt), stat = "identity", fill = colors[1], alpha = 0.8) +
    # geom_bar(aes(y = det), stat = "identity", fill = colors[2], alpha = 0.8)  +
    labs(
      x = "Month of Year",
