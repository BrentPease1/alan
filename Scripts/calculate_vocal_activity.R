...
for(c in continents){
  ...
  for(m in 1:nrow(months)){
    ...
    if("station.type" %in% names(bw)){
      bw <- bw[station.type != "stream_audio",]
    }
      ...
      total_spp <- total_spp[, .N, by = .(`Common Name`, station_date)]
        ...
        for(dets in detection_filter){
          ...
          for(f in focal_spp){
            ...
            # plot time stamps, if needed
            # single_spp_dets[, count := .N, by = time_local_real]
            # ggplot(data = single_spp_dets ) +
            #   geom_line(aes(x = time_local_real, y = count), 
            #             size = 1) +
            #   labs(x = "Date", 
            #        y = "activity",
            #        title = "") +
            ...
            first_onset <- single_spp_dets[time_local_real >= midnight & time_local_real < hms::as_hms(solarNoon),]
            ...
            # --- # --- # --- # --- # --- # --- # --- # --- # --- # --- 
            # median activity
            med_voc <- single_spp_dets[time_local_real >= midnight & time_local_real < hms::as_hms(solarNoon),]
            ...
            out <- merge(out, single_spp_dets[!duplicated(station_date), .(station_date, Station, Latitude, Longitude, week, date = date(dt_local_real))])
